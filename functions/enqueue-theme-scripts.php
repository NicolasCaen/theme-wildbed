...
add_action('after_setup_theme', 'wildbed_editor_styles_setup');
  ...
  function enqueue_external_link_target() {
    wp_enqueue_script('external-link-target', get_stylesheet_directory_uri() . '/assets/js/external-link-target.js', [], '', true);
  }
  add_action('wp_enqueue_scripts', 'enqueue_external_link_target');