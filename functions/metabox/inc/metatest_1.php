...
class UGM_meta_test extends UGM_Metabox {

    private static $instance = null;
    private $metabox_id = 'metatest_1';
    private $metabox_title = 'MEta test';
    private $post_types = array('page');
    ...
    public function register_meta_fields() {
        foreach ($this->post_types as $post_type) {
            register_post_meta($post_type, 'blah', array(
                'single' => true,
                'type' => 'string',
                'show_in_rest' => true,
        ...
        foreach ($this->taxonomies as $taxonomy) {
            register_term_meta($taxonomy, 'blah', array(
                'single' => true,
                'type' => 'string',
                'show_in_rest' => true,
                'auth_callback' => function() { return current_user_can('edit_posts'); }
    ...
    public function render_metabox($post) {
        ...
        // Champ: blah
        $value_blah = get_post_meta($post->ID, 'blah', true);
        echo '<tr>';
        echo '<th scope="row"><label for="blah">blah</label></th>';
        echo '<td>';
        echo '<input type="text" id="blah" name="blah" value="' . esc_attr($value_blah) . '" class="regular-text" />';
        echo '</td>';
        echo '</tr>';
    ...
    public function save_metabox($post_id) {
        ...
        // Sauvegarder: blah
        if (isset($_POST['blah'])) {
            $value_blah = sanitize_text_field(wp_unslash($_POST['blah']));
        } else {
            $value_blah = '';
        }
        update_post_meta($post_id, 'blah', $value_blah);
    ...
    public function render_taxonomy_add_fields($taxonomy) {
        ...
        echo '<label for="blah">blah</label>';
        $this->render_taxonomy_field('blah', 'text', '');
        echo '</div>';
        echo '</div>';
    }
    ...
    public function render_taxonomy_edit_fields($term, $taxonomy) {
        ...
        $value_blah = get_term_meta($term->term_id, 'blah', true);
        echo '<tr class="form-field" data-ugm-binding=\"1\">';
        echo '<th scope="row"><label for="blah">blah</label></th>';
        echo '<td>';
        $this->render_taxonomy_field('blah', 'text', $value_blah);
        echo '</td></tr>';
    }
    ...
    public function save_taxonomy_meta($term_id, $tt_id = 0) {
        ...
        update_term_meta($term_id, 'blah', $value_blah);
...
if (class_exists('UpGutenbergMetabox')) {
    ...
    $ugm_source = isset($ugm_sources['metatest_1']) ? $ugm_sources['metatest_1'] : 'plugin';
    $ugm_should_load = ($ugm_source === 'theme');
}
if ($ugm_should_load) {
    UGM_meta_test::get_instance();
}
